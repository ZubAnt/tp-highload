...
class FileReader(object):
    ...
    async def read(self, filename: str) -> bytes:
        ...
        # file.close()
        # return data
        return await self._loop.run_in_executor(self._executor, self._open_and_read, filename)

    def _open_and_read(self, filename: str) -> bytes:

        data: bytes = b""
        with open(filename, 'rb') as file:
            while True:
                logging.debug(f"[FileReader] try read chunk...")
                chunk = self._read_chunk_sync(file, self._conf.read_chunk_size)
                data += chunk
                logging.debug(f"[FileReader] chunk: {chunk}")
                if chunk == b'':
                    break
    ...
    @classmethod
    def _read_chunk_sync(cls, file, chunk_size):
